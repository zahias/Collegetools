...
# ---------------------- Parsing helpers (same core behavior) ----------------------
...
# ---------------------- New helpers for splitting files ----------------------
def _norm_col_name(s: str) -> str:
    return re.sub(r'[^a-z0-9]', '', s.strip().lower())

def _find_col(df: pd.DataFrame, candidates: List[str]) -> Optional[str]:
    """Find a column by a set of candidate normalized names."""
...
def _year_from_id(val) -> Optional[int]:
    ...
    if not s:
        return None
    # Prefer first 4 digits at the start; else first 4-digit sequence anywhere
    m = re.match(r'^(\d{4})', s)
    if not m:
        m = re.search(r'(\d{4})', s)
    if not m:
...
def _is_pbhl(major_val: str) -> bool:
    ...
    # Accept PBHL prefix or known alt (PUBHEA, PUBLIC HEALTH)
...
def _is_spth(major_val: str) -> bool:
    ...
    # Match SPTH / SLP / SPEECH*THERAPY variants
    return "SPTH" in m or "SPEECH" in m or "SLP" in m

def split_three_outputs(tidy_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Optional[str], Optional[str]]:
    """
    Returns: (pbhl_df, spth_old_df, spth_new_df, id_col, major_col)
    If id/major missing, pbhl_df/spth_* may be empty.
    """
    if tidy_df.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), None, None

    # Detect ID and MAJOR columns from the tidy df (case-insensitive, robust)
...
def run():
    ...
    if up:
        try:
            ...
            if id_like_cols:
                with c3: st.metric("Unique Students", tidy_df[id_like_cols[0]].nunique())
            else:
            ...
            # --- Always allow download of the full cleaned file
            full_out = BytesIO()
            ...
            # --- Build the 3 requested filtered files
